...
class GeminiProvider(AIProvider):
        ...
        try:
            ...
            with httpx.Client(timeout=15.0) as client:
                ...
                if res.status_code == 200:
                    ...
                    logger.warning("Gemini API returned empty candidates, falling back to Mock AI.")
                    return None
                else:
                    logger.warning(f"Gemini API returned status {res.status_code}: {res.text[:200]}, falling back to Mock AI.")
                    return None
        except Exception as e:
            logger.warning(f"Error calling Gemini API: {e}, falling back to Mock AI.")
            return None
    ...
    def investigate_incident(
        ...
        base_res["analyst_notes"] = f"[Gemini AI] {llm_reply[:300]}"
        return base_res
    ...
    def generate_response_plan(
        ...
        if reply:
            try:
                ...
                logger.warning("Failed to parse Gemini response plan JSON, falling back to Mock AI.")
        return self.mock_fallback.generate_response_plan(incident, events)
