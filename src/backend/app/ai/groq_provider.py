...
class GroqProvider(AIProvider):
    """
    Groq AI provider — ultra-fast inference using Groq's OpenAI-compatible API.
    Supports models like llama-3.3-70b-versatile, mixtral-8x7b-32768, gemma2-9b-it.
        ...
        try:
            ...
            with httpx.Client(timeout=10.0) as client:
                ...
                if res.status_code == 200:
                    ...
                    logger.warning(f"Groq API returned status {res.status_code}: {res.text[:200]}, falling back to Mock AI.")
                    return None
        except Exception as e:
            logger.warning(f"Error calling Groq API: {e}, falling back to Mock AI.")
            return None
    ...
    def investigate_incident(
        ...
        base_res["analyst_notes"] = f"[Groq AI] {llm_reply[:300]}"
        return base_res
    ...
    def generate_response_plan(
        ...
        reply = self._call_llm(
            "You are CyberSentinel SOC AI. Generate a JSON array of recommended response actions. "
            "Each action has: title (str), description (str), priority (CRITICAL/HIGH/MEDIUM/LOW), "
            "action_type (ISOLATE/BLOCK/MONITOR/INVESTIGATE/ESCALATE). Return ONLY valid JSON array.",
            f"Incident: {incident.title} (Severity: {incident.severity})\nEvents:\n{events_summary}\n\n"
            "Generate 3-5 non-destructive response actions as a JSON array."
        )
        if reply:
            try:
                clean = reply.strip()
                ...
                logger.warning("Failed to parse Groq response plan JSON, falling back to Mock AI.")
        return self.mock_fallback.generate_response_plan(incident, events)
    ...
    def generate_report(
        ...
        events_summary = "\n".join([f"- {e.timestamp.strftime('%H:%M:%S')}: {e.event_type} ({e.description})" for e in events[:20]])
        reply = self._call_llm(
            "You are CyberSentinel SOC AI. Generate a comprehensive, professional SOC incident report in Markdown format. "
            "Include sections: Executive Summary, Timeline, Technical Analysis, Impact Assessment, Recommended Actions.",
        ...
        return reply or self.mock_fallback.generate_report(incident, events, investigation)
